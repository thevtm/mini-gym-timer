...
var $author$project$App$init = function (_v0) {
	return _Utils_Tuple2(
		{exerciseTimer: 40, prepareTimer: 10, state: $author$project$App$Idle},
		$elm$core$Platform$Cmd$none);
};
...
var $author$project$App$Finished = {$: 'Finished'};
		...
		switch (msg.$) {
			...
			case 'Tick':
				...
				switch (_v1.$) {
					case 'Prepare':
						var seconds = _v1.a;
						return (_Utils_cmp(seconds, model.prepareTimer) > -1) ? _Utils_Tuple2(
							_Utils_update(
								model,
								{
									state: $author$project$App$Exercise(0)
								}),
...
var $author$project$App$toString = function (model) {
	...
	switch (_v0.$) {
		...
		case 'Prepare':
			var seconds = _v0.a;
			return 'starting in ' + ($elm$core$String$fromInt(model.prepareTimer - seconds) + ' seconds');
		case 'Exercise':
			var seconds = _v0.a;
			return 'running for ' + ($elm$core$String$fromInt(seconds) + ' seconds');
		default:
			return 'finished';
	}
};
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$text(
						'Status: ' + $author$project$App$toString(model))
					])),
				A2(
				$elm$html$Html$button,
				_List_fromArray(
...
_Platform_export({'App':{'init':$author$project$App$main(
	$elm$json$Json$Decode$succeed(_Utils_Tuple0))({"versions":{"elm":"0.19.1"},"types":{"message":"App.Msg","aliases":{},"unions":{"App.Msg":{"args":[],"tags":{"Start":[],"Finish":[],"Tick":["Time.Posix"]}},"Time.Posix":{"args":[],"tags":{"Posix":["Basics.Int"]}},"Basics.Int":{"args":[],"tags":{"Int":[]}}}}})}});}(this));